        ...
        public ImagePanel() {
            addMouseListener(new MouseAdapter() {
                ...
                public void mousePressed(MouseEvent e) {
                    ...
                    zoomRect.width = 1;
                    zoomRect.height = 1;
                    //System.out.println("mousePressed: zommRect[x=" + zoomRect.x + ",y=" + zoomRect.y +
    ...
    */
    ...
    public class ClosingWindowListener extends WindowAdapter {
		public void windowClosed(WindowEvent e) {
            if (! hasMarkedElements(model)) {
                return;
            }
            final File dir = new File(OUTDIR);
            if (!dir.renameTo(new File(dir.toString() + "." + System.currentTimeMillis()))) {
                System.err.println("Renaming of /tmp/pictures failed");
            }
            dir.mkdir();
            ...
            System.out.println("Marked files saved in " + dir.toString());
        }

        private boolean hasMarkedElements(DefaultListModel model) {
            for (int i=0; i < model.size(); i++) {
                Element e = (Element) model.getElementAt(i);
                if (e.isMark()) {
                    return true;
                }
            }
