...
public class NavigableImagePanel extends JPanel {
	...
	private static final double SCREEN_NAV_IMAGE_FACTOR = 0.15; // 15% of
																// panel's width
	...
	public void setImage(BufferedImage image) {
		...
		this.image = image;
		// Reset scale so that initializeParameters() is called in
		// paintComponent()
		// for the new image.
		scale = 0.0;
		firePropertyChange(IMAGE_CHANGED_PROPERTY, (Image) oldImage,
				(Image) image);
		repaint();
	}
	...
	// Converts the navigation image coordinates into the zoomed image
	// coordinates
	...
	// Tests whether a given point in the panel falls within the image
	// boundaries.
	...
	// Tests whether a given point in the panel falls within the navigation
	// image
	// boundaries.
	...
	protected void paintComponent(Graphics g) {
		...
		if (true || isHighQualityRendering()) {
			...
			if (rect == null || rect.width == 0 || rect.height == 0) { // no
																		// part
																		// of
																		// image
																		// is
			...
			BufferedImage subimage = image.getSubimage(rect.x, rect.y,
					rect.width, rect.height);
			Graphics2D g2 = (Graphics2D) g;

			// added by klaus START - rotate
			...
			// added by klaus END - rotate

			g2.setRenderingHint(RenderingHints.KEY_INTERPOLATION,
					INTERPOLATION_TYPE);
			g2
					.drawImage(subimage, Math.max(0, originX), Math.max(0,
							originY), Math.min(
							(int) (subimage.getWidth() * scale), getWidth()),
							Math.min((int) (subimage.getHeight() * scale),
									getHeight()), null);
		} else {
	...
	private void drawZoomAreaOutline(Graphics g) {
		...
		int width = getWidth() * getScreenNavImageWidth()
				/ getScreenImageWidth();
